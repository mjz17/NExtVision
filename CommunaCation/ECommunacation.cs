...
namespace CommunaCation
{
    ...
    public class ECommunacation
    {
        ...
        public bool Connect()
        {
            ...
            switch (CommunicationModel)
            {
                case CommunicationModel.TcpClient:
                    ...
                    IsConnected = !m_DMTcpClient.Isclosed;
                    IsConnected = true;//2023.2.23,赵一添加
        ...
        private void M_DMTcpClient_OnStateInfo(string msg, SocketState state)
        {
            ...
            switch (state)
            {
                ...
                case SocketState.Connected:
                    ...
                    SysHelper.DeviceInfo info_0 = new SysHelper.DeviceInfo()
                    {
                        DeviceName = Key,
                        IsConnected = true,
                    };

                    SysHelper.DataEventChange.DeviceChange = info_0;

                    Debug.WriteLine($"已成功连接服务器  {RemoteIP}:{RemotePort}");
                    break;
                case SocketState.Reconnection://重新连接服务器
                    SysHelper.DeviceInfo info_1 = new SysHelper.DeviceInfo()
                    {
                        DeviceName = Key,
                        IsConnected = false,
                    };

                    SysHelper.DataEventChange.DeviceChange = info_1;
                    Debug.WriteLine($"重新连接服务器  {RemoteIP}:{RemotePort}");
                    break;
                case SocketState.Disconnect:
                    if (IsHasObjectConnected == true)
                    {
                        ...
                        SysHelper.DeviceInfo info_2 = new SysHelper.DeviceInfo()
                        {
                            DeviceName = Key,
                            IsConnected = false,
                        };

                        SysHelper.DataEventChange.DeviceChange = info_2;
        ...
        internal void OnDeSerializingMethod(StreamingContext context)
        {
            ...
            m_RecStrQueue = new Queue<string>();
            IsConnected = false;
        }
    }
}
